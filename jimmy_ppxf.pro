...
for i = 0, max(binNum) do begin
    ...
    if ( monte_iterations ne 0 ) then begin
        ...
        for k=0,monte_iterations-1 do begin
            ...
            if CanConnect() then begin
	            ppxf, templates, noisy_galaxy, noise, velScale, start, sol, GOODPIXELS=goodPixels, /PLOT, MOMENTS=4, DEGREE=6, VSYST=dv,BIAS=0.253, ERROR=error
	        endif else begin
	        	ppxf, templates, noisy_galaxy, noise, velScale, start, sol, GOODPIXELS=goodPixels, MOMENTS=4, DEGREE=6, VSYST=dv,BIAS=0.253, ERROR=error
	        endelse
    ...
    if CanConnect() then begin
	    ...
	    ppxf, templates, galaxy, noise, velScale, start, sol, GOODPIXELS=goodPixels, /PLOT, MOMENTS=4, DEGREE=8, VSYST=dv,BIAS=0.253, ERROR=error
	    device,/close
	endif else begin
	    ppxf, templates, galaxy, noise, velScale, start, sol, GOODPIXELS=goodPixels, MOMENTS=4, DEGREE=4, VSYST=dv,BIAS=0.253, ERROR=error
	endelse
